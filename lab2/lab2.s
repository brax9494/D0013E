...
then2:	lw    $t0, -4($fp)   # takes the value of "x" from the stack and stores it on the stack, but where we want "r" to be stored. We do this because it's the base case.
	sb    $zero, 0($a1)
	sw    $t0, -8($fp)
	b     exit2 
	addiu $v0, $t0, 0
